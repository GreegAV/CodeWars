...
the percentage of foam or gas lost every day (evap_per_day) and the threshold (threshold) in percentage beyond which the evaporator is no longer useful. All numbers are strictly positive.
...
public class Kata016{
    public static int evaporator(double content, double evap_per_day, double threshold) {
        // your code
        int days=0;
        double rest=content*threshold/100;
        while (rest <= content){
            days++;
            content-=(content/100.0)*evap_per_day;
        }
        return days;
    }
}
